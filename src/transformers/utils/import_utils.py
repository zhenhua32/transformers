...
def is_sentencepiece_available():
    # 检查模块是否存在, 存在的话会返回一个 ModuleSpec 对象
...
class DummyObject(type):
    ...
    定义一个元类, 当用户尝试访问任何属性时, 会抛出 ImportError 异常.
    """
...
class _LazyModule(ModuleType):
    ...
    模块懒加载
    继承自 ModuleType, ModuleType = type(sys)
    """
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        # 设置属性, 下次就不需要重复加载了. 会保存在 __dict__ 中
    ...
    def _get_module(self, module_name: str):
        """主要的加载过程"""
        try:
            # 相当于 from {self.__name__} import {module_name} 语法
            print("加载了", module_name)
